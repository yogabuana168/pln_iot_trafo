...
function updateSimpleBar(value) {
    // Avoid re-writing sidebar HTML to prevent flicker
    ...
    if ((value === 'vertical' || value === 'horizontal' || value === 'semibox') && !isIcon) {
        if (!sidebarSimpleBarMenu.getAttribute('data-simplebar')) {
            sidebarSimpleBarMenu.setAttribute('data-simplebar', '');
            if (window.SimpleBar) new SimpleBar(sidebarSimpleBarMenu);
        }
    } else {
        if (window.SimpleBar) {
            const simpleBarInstance = SimpleBar.instances.get(sidebarSimpleBarMenu);
            if (simpleBarInstance) simpleBarInstance.unMount();
        }
        // collapse any open submenus in icon mode
...
// Helper: when sidebar is in icon mode, make the Dashboards top item a direct link to /dashboard
function updateDashboardTriggerForIconMode() {
    const isIcon = document.documentElement.getAttribute("data-sidebar") === "icon";
    const dashTrigger = document.querySelector('#sidebar a.pe-nav-link[aria-controls="collapseDashboards"]');
    if (!dashTrigger) return;
    const dashLi = dashTrigger.closest('li');
    const arrow = dashTrigger.querySelector('.pe-nav-arrow');
    if (isIcon) {
        dashTrigger.setAttribute('href', '/dashboard');
        dashTrigger.removeAttribute('data-bs-toggle');
        if (dashLi) dashLi.classList.remove('pe-has-sub');
        if (arrow) arrow.style.display = 'none';
    } else {
        dashTrigger.setAttribute('href', '#collapseDashboards');
        dashTrigger.setAttribute('data-bs-toggle', 'collapse');
        if (dashLi && !dashLi.classList.contains('pe-has-sub')) dashLi.classList.add('pe-has-sub');
...
function closeHeaderDropdowns() {
    document.querySelectorAll('header .dropdown-menu.show').forEach((el) => {
